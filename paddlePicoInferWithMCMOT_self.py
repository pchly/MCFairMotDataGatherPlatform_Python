...
def print_arguments(args):
    ...
    for arg, value in sorted(vars(args).items()):
        print('%s: %s' % (arg, value))
    print('------------------------------------------')
def init_Pico_detector(p_model_dir):
    deploy_file = os.path.join(p_model_dir, 'infer_cfg.yml')
    with open(deploy_file) as f:
        yml_conf = yaml.safe_load(f)
    arch = yml_conf['arch']
    label_list = yml_conf['label_list']
    ...
    detector = eval(detector_func)(
        ...
        device="GPU",
        output_dir="output/roi_pico_output")
    return detector ,label_list
def runPicTest(detector,p_frame):
    # img_list = get_test_images('./', p_input_img_path)
    # img_list = [p_input_img_path]
    results = detector.predict_image([p_frame], repeats=100)
    detector.det_times.info(average=True)
    return results
def pico_det_rect(np_boxes,threshold):
...
if __name__ == '__main__':
    ...
    runPicTest("./inference_model/picodet_l_640_coco_lcnet",'/home/aistudio/work/testImgs/09.jpg')
    # main()
