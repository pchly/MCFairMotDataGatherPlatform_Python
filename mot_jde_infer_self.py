...
sys.path.insert(0, pico_path)
# print("parent_path:",pico_path)
# print("os.path.abspath(__file__)",os.path.abspath(__file__))
from paddlePicoInferWithMCMOT_self import init_Pico_detector,runPicTest,pico_det_rect
...
class JDE_Detector(Detector):
    ...
    def predict_video(self, video_file, camera_id):
        ...
        video_fps = fps
        pico_detector , label_list_roi = init_Pico_detector("./inference_model/picodet_l_640_coco_lcnet")
        while (1):
            ...
            if len(online_tlwhs) > 0:
                ...
                for i in online_tlwhs:
                    ...
                    h = int(online_tlwhs[i][0][3]);
                    if x >=0 and y >=0 and w>=0 and h >=0:
                        roi_frame = frame[y:y+h,x:x+w]
                        cv2.imwrite(roi_img_path,roi_frame)
